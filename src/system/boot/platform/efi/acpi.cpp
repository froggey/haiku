...
{
	#if 0
	addr_t pageOffset = physicalAddress & (EFI_PAGE_SIZE - 1);

	physicalAddress -= pageOffset;
	size += pageOffset;
	
	EFI_STATUS status =
		kSystemTable->BootServices->AllocatePages(AllocateAddress, EfiLoaderData,
			size / EFI_PAGE_SIZE + 1, &physicalAddress);
	if (status != EFI_SUCCESS)
		panic("mmu_map_physical_memory failed: %d; phys address = %lx, size = %ld",
			status, physicalAddress, size);
	#endif
...
{
	TRACE(("mmu_free: not implemented\n"));
}
